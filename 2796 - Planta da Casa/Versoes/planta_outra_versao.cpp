...
void seleciona_melhor_segunda(casa *c, vector <mesa> *pm, int area_retangulo)
{
	int a = 0, l = 0, index = 0, k=0, p=0;
	for(auto const &i:c->mesas)
	{
		for(int j=0;j<pm->size();j++)
		{
			if((i.largura <= pm->at(j).largura && 
				i.comprimento <= pm->at(j).comprimento) ||
				(i.comprimento <= pm->at(j).largura && 
				i.largura <= pm->at(j).comprimento)) // aqui por causa dos 90º
			{
				if(i.area > a || (i.area == a && i.largura > l))
				{
					a = i.area;
...
void leitura_montagem_casa(casa *c)
{
	...
	*/
	...
	vector <mesa> pontos{pontos_maximos[p.ia], pontos_maximos[p.ic], pontos_maximos[p.il]};
	...
	for(i=0;i<c->quant_mesas;i++)
	{
		...
		((m.comprimento <= p.c && m.largura <= p.l) || 
		(m.largura <= p.c && m.comprimento <= p.l)) && 
	...
	seleciona_melhor_segunda(c, &pontos, area_retangulo);
}
int main()
{
